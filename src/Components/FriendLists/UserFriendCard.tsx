...
import { IFriendListItem, getSelfData, getSingleUser, getUserList } from '../../Services/User.service';
...
const UserFriendCard = ({dbId}: any) => {
  ...
  return (
    ...
    <div className="flex items-center justify-between mb-5">
      ...
      <div>
          <div>
            <button className={portfolioValue < 0 ? "bg-red-200 py-2 px-2 rounded-full text-red-800 text-xs font-semibold" : "bg-green-200 py-2 px-2 rounded-full text-green-800 text-xs font-semibold"} >
             {`${Number(portfolioValue)> 0? '+':''} ${Number(portfolioValue).toFixed(2)}`}
            </button>
          </div>
        </div>
    </div>
