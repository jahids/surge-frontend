...
export const getUserList = async ()=>{
    try {
        const {data :{data : result}} = await instance.get(`/social/people`);
        // console.log(result);
        let filteredResult : [IFriendListItem] = result.map( (v : any) => {
            const item  : IFriendListItem= {
                alpaca_id : v.id as string,
                name : (v?.identity.given_name + " " +  v?.identity.family_name) as string,
