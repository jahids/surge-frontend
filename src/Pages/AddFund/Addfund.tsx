...
function Addfund() {
  ...
  useEffect(() => {
    const singleItemCall = async () => {
      try {
        ...
        setLoading(false);
        console.log('🔥response', achData.data.data[0]);
      } catch (error) {
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      console.log(error);
      const axiosErrorMessage = (error as AxiosError).response?.data?.error?.message;
      notifyError(axiosErrorMessage || 'Transfer request Failed');
    }
  };
  if (loading) {
    return <Loader />
              ...
              {/* <div className="h-screen flex justify-center items-center bg-gray-100">
                <div className="bg-white p-8 rounded-lg shadow-md w-96">
                  <div className="flex flex-col items-center">
                    <div className="bg-gray-300 h-24 w-24 mb-4 rounded-full flex items-center justify-center">
                      <span className="text-6xl text-gray-400">!</span>
                    </div>
                    <h1 className="text-2xl font-semibold mb-4">No Data Available</h1>
                    <p className="text-gray-600 text-center mb-6">It seems there's nothing here yet. Check back later or try a different filter.</p>
                    <button className="bg-blue-500 text-white px-6 py-2 rounded hover:bg-blue-600 transition duration-200">Go Back</button>
                  </div>
                </div>
              </div> */}

              <div className="" style={{ display: 'flex', flexDirection: 'column', height: '80vh', width: "90vw", alignItems: 'center', justifyContent: 'center' }} >
                ...
                <h2 className='text-xl font-bold center'>No Bank Linked!</h2>
                <button onClick={() => navigate('/main')} className="mt-4 btn btn-primary">Add Bank</button>
              </div>
            </div>
